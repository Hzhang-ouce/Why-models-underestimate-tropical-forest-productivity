...
Jfun_package_original <- function(PPFD, alpha, Jmax, theta){
  (alpha*PPFD + Jmax - 
     sqrt((alpha*PPFD + Jmax)^2 - 4*alpha*theta*PPFD*Jmax))/(2*theta)
}

Jfun <- function(PPFD, alpha, Jmax, theta){
  alpha*PPFD/sqrt(1+(alpha*PPFD/Jmax)^2)
}
# The is actually equation (10) in 'Global photosynthetic capacity is optimized to the environment'
# Given PPFD and J, what is Jmax? (inverse non-rectangular hyperbola)
inverseJfun_package_original <- function(PPFD, alpha, J, theta){
  J*(J*theta - alpha*PPFD)/(J - alpha*PPFD)
}

inverseJfun <- function(PPFD, alpha, J, theta){
  sqrt(1/(1/J^2 - 1/ (alpha*PPFD)^2 ))
}
